...
async function deletePhotoById(req, res) {
    try {
        ...
        logger.info(`Photo ${id} deleted by user ${req.user.id}`);
...
async function deletePhotosInBulk(req, res) {
    ...
    if (!Array.isArray(photoIds) || photoIds.length === 0) {
        logger.warn(`Bulk delete attempt with invalid photoIds by user ${req.user.id}`);
        return res.status(400).json({ error: 'photoIds must be a non-empty array' });
    }

    try {
        const result = await deleteMultiplePhotos(photoIds);
        logger.info(`Bulk photo deletion by user ${req.user.id}. Success: ${result.deleted}, Errors: ${result.errors}`);
